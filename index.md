...
runs a complete audit of a `summarised_result` object covering
suppression status, package metadata, and estimate types.
...
reports the suppression status for each `result_id` — which results have
been suppressed, with what minimum cell count, and how many rows are
affected.
...
reports which package name and version produced each result, broken down
by result type.
...
lists the estimate types (e.g. `count`, `mean`, `sd`) present in each
result type, along with their row counts.
...
It allows you to upload a `summarised_result` `.csv` file and
interactively explore and validate its contents.
